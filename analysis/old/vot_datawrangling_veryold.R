...
MTS <- MTS[TRPDUR<120 & TRPDUR>0, ]

#Trip dist breaks
dists <- c(DIST1=0,DIST2=0.5,DIST3=1,DIST4=2,DIST5=4,DIST6=8,DIST7=20,END=128)
...
durs <- c(DUR1=0,DUR2=5,DUR3=10,DUR4=20,DUR5=40,DUR6=60,END=128)
for(i in 1:(length(durs)-1)) MTS[TRPDUR >= durs[i] & TRPDUR < durs[i+1], TRPDUR_GRPD := names(durs[i])]

ggplot(MTS[!(MODE %in% 1:2),],aes(x=TRPDUR_GRPD)) + geom_bar()
...
MTS[TPURP %in% c(8,9,10,11,96,97), TRPPUR := "TRAVEL"]
...
save(mnldt, MTS, file = "dat_mnldt.RData")
