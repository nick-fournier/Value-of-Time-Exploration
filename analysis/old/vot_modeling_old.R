...
mnlfullmod <- mlogit(CHOICE~1|AGE+INCOME+TRPPUR+TRPTIME|COST+TRPDUR, data=mnldt,
...
t(combn(c("INCOME","TRIPDIST","AGE","TRPPUR","TRPTIME"), 2))

#Estimating them
mnlmodlistjt[['INCAGE']] <- mlogit(CHOICE~TRPPUR|(COST+TRPDUR):INCOME:AGE_GRPD, data=mnldt,
                                   shape='long', alt.var='MODE', choice='CHOICE', reflevel='DRIVE')
...
votmodlistjt <- lapply(names(mnlmodlistjt), function(varname) {
  ...
  votmodjt[ , rn := gsub(paste("^COST:|:COST$|^TRPDUR:|:TRPDUR$|TRPDIST_GRPD|AGE_GRPD|TRPPUR|TRPTIME",'',sep="|"),"",rn)]
  ...
  #Merging Frequencies
  nage = mnldt[CHOICE==T,.N, by='AGE_GRPD']
  ndist = mnldt[CHOICE==T,.N, by='TRPDIST_GRPD']
  ninc = mnldt[CHOICE==T,.N, by='INCOME'][ , .(INCOME=paste("INCOME",INCOME,sep=""),N)]
  npurp = mnldt[CHOICE==T,.N, by='TRPPUR']
  ntime = mnldt[CHOICE==T,.N, by='TRPTIME']
  
  
  #Which column is each variable in, V1 or V2?
  col = colnames(votmodjt)[which(sapply(votmodjt, function(x) all(grepl("AGE",x))) & colnames(votmodjt)!="VARGROUP")]
  if(length(col)>0){
    votmodjt <- merge(votmodjt, nage, by.=col, by.y='AGE_GRPD', all = T)
    votmodjt <- merge(votmodjt, varlabelvals[['AGE_GRPD']], by.x = col, by.y = "VAR", all = T)
    setnames(votmodjt, c('LABEL','VAL','N'), paste(col,c('LABEL','VAL','N'),sep="_"))
  }
  
  col = colnames(votmodjt)[which(sapply(votmodjt, function(x) all(grepl("DIST",x))) & colnames(votmodjt)!="VARGROUP")]
  if(length(col)>0){
    votmodjt <- merge(votmodjt, ndist, by.=col, by.y='TRPDIST_GRPD', all = T)
    votmodjt <- merge(votmodjt, varlabelvals[['TRPDIST_GRPD']], by.x = col, by.y = "VAR", all = T)
    setnames(votmodjt, c('LABEL','VAL','N'), paste(col,c('LABEL','VAL','N'),sep="_"))
  }
  
  col = colnames(votmodjt)[which(sapply(votmodjt, function(x) all(grepl("INCOME",x))) & colnames(votmodjt)!="VARGROUP")]
  if(length(col)>0) {
    votmodjt <- merge(votmodjt, ninc, by.=col, by.y='INCOME', all = T)
    votmodjt <- merge(votmodjt, varlabelvals[['INCOME']][ , .(VAR=paste("INCOME",VAR,sep=""),LABEL,VAL)], by.x = col, by.y = "VAR", all = T)
    setnames(votmodjt, c('LABEL','VAL','N'), paste(col,c('LABEL','VAL','N'),sep="_"))
  }
  
  col = colnames(votmodjt)[which(sapply(votmodjt, function(x) 
    all(grepl(paste(varlabelvals$TRPPUR$VAR, collapse = "|"),x))) & colnames(votmodjt)!="VARGROUP")]
  if(length(col)>0){
    votmodjt <- merge(votmodjt, npurp, by.=col, by.y='TRPPUR', all = T)
    votmodjt <- merge(votmodjt, varlabelvals[['TRPPUR']], by.x = col, by.y = "VAR", all = T)
    setnames(votmodjt, c('LABEL','VAL','N'), paste(col,c('LABEL','VAL','N'),sep="_"))
  }
  
  col = colnames(votmodjt)[which(sapply(votmodjt, function(x) 
    all(grepl(paste(varlabelvals$TRPTIME$VAR, collapse = "|"),x))) & colnames(votmodjt)!="VARGROUP")]
  if(length(col)>0){
    votmodjt <- merge(votmodjt, ntime, by.=col, by.y='TRPTIME', all = T)
    votmodjt <- merge(votmodjt, varlabelvals[['TRPTIME']], by.x = col, by.y = "VAR", all = T)
    setnames(votmodjt, c('LABEL','VAL','N'), paste(col,c('LABEL','VAL','N'),sep="_"))
  }
...
votmodlistjtint <- lapply(names(mnlmodlistjt), function(varname) {
  #Extracting coefficients & merging standard errors
  votmodjt <- as.data.table(summary(mnlmodlistjt[[varname]])$CoefTable,keep.rownames = T)#[grepl("COST|TRPDUR", rn), ]
  #cleanup the names
  votmodjt[grepl('^COST:|:COST$',rn), VOTVAR := "COST"]
...
names(votmodlistjtint) <- names(votmodlistjtint)
votmodjtint <- rbindlist(votmodlistjtint,use.names = T)

### Reshaping individual models
modnames <- names(mnlmodlist)
votmodlist <- lapply(modnames, function(varname) {
...
#### Savings ####
#Cleanup
save(mnldt, votavg, votmoddt, votmodjt, votmodjtint, votmodlist, votmodlistjt, 
     mnlavgmod, mnlfullmod, mnlmodlist, mnlmodlistjt, varlabelvals,
     file = "./analysis/dat_estimate.RData")
